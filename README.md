...
- PC常駐プロセス、Node.js、ローカルブラウザ自動操作は使いません。
- ログイン情報は保存しません。認証は各社の公式サイト側で行います。
- iPhoneで公式サイトを開き、取得したファイルをアプリに戻って手動登録します。
...
- `対象月を確認 → 公式サイトを開く → ファイル名を確認 → Driveへ保存` のフロー
...
2. 未取得のセルを押すか、`取得開始` タブでサービスと対象月を選びます。
3. 公式サイトを開き、領収書・明細をiPhoneでダウンロードします。
4. `手動登録` タブに戻り、ファイル、取引日、取引先、金額、取得元URLを入力します。
5. `Driveへ保存` を押すと、Google Driveへ保存され、台帳に記録されます。
